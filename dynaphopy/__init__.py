...
class Quasiparticle:
    ...
    def save_vc_hdf5_all_q(self, symmetry=False):
        ...
        if self.parameters.project_on_atom > -1:
            element = self.dynamic.structure.get_atomic_elements(unique=True)[
                self.parameters.project_on_atom
            ]
            print(
                "Project on atom {} : {}".format(
                    self.parameters.project_on_atom, element
                )
            )
        ...
        if symmetry:
            ...
            qpoints = pho_interface.get_commensurate_points(
                self.dynamic.structure, md_supercell
            )
        ...
        assert (
            self._traj_file is not None
        ), "ERROR: evaluation of vc for all q-points is only supported in on-the-fly reading mode."
        ...
        self.dynamic.set_time(
            np.linspace(
                0,
                (all_vc[0].shape[0] - 1) * self.dynamic.get_time_step_average(),
    ...
    def _set_frequency_range(self, frequency_range):
        if not np.array_equiv(
            np.array(frequency_range), np.array(self.parameters.frequency_range)
        ):
            self.power_spectra_clear()
    ...
    def set_spectra_resolution(self, resolution):
        ...
        self._set_frequency_range(
            np.arange(limits[0], limits[1] + resolution, resolution)
        )
        ...
        if self._eigenvectors is None:
            ...
            if self._phanafile is None:
                self._eigenvectors, self._frequencies = (
                    pho_interface.obtain_eigenvectors_and_frequencies(
                        self.dynamic.structure, self.parameters.reduced_q_vector
                    )
                )
            elif self._hdf5_file is not None:
                self._eigenvectors, self._frequencies = (
                    pho_interface.obtain_hdf5_eigenvectors_and_frequencies(
                        self.parameters.reduced_q_vector, self._hdf5_file
                    )
                )
            else:
                self._eigenvectors, self._frequencies = (
                    pha_interface.obtain_phana_eigenvectors_and_frequencies(
                        self.parameters.reduced_q_vector, fname=self._phanafile
                    )
                )
    ...
    def get_band_ranges_and_labels(self):
        ...
        if self.parameters.band_ranges is None:
            self.parameters.band_ranges = (
                self.dynamic.structure.get_path_using_seek_path()
            )
    ...
    def plot_phonon_dispersion_bands(self):
        ...
        if self._bands is None:
            self._bands = pho_interface.obtain_phonon_dispersion_bands(
                self.dynamic.structure, band_ranges, NAC=self.parameters.use_NAC
            )
    ...
    def plot_renormalized_phonon_dispersion_bands(
        self, plot_linewidths=False, plot_harmonic=True
    ):

        bands_full_data = self.get_renormalized_phonon_dispersion_bands(
            with_linewidths=plot_linewidths
        )
        ...
        for i, path in enumerate(bands_full_data):
            ...
            plt.plot(
                path["q_path_distances"],
                np.array(list(path["renormalized_frequencies"].values())).T,
                color="r",
                label="Renormalized",
            )
            ...
            if plot_harmonic:
                plt.plot(
                    path["q_path_distances"],
                    np.array(list(path["harmonic_frequencies"].values())).T,
                    color="b",
                    label="Harmonic",
                )

            if plot_linewidths:
                for freq, linewidth in zip(
                    list(path["renormalized_frequencies"].values()),
                    list(path["linewidth"].values()),
                ):
                    plt.fill_between(
                        path["q_path_distances"],
                        freq + np.array(linewidth) / 2,
                        freq - np.array(linewidth) / 2,
                        color="r",
                        alpha=0.2,
                        interpolate=True,
                    ...
                    plot_title = (
                        "Renormalized phonon dispersion relations and linewidths"
                    )
    ...
    def plot_frequencies_vs_linewidths(self):
        ...
        qpoints, multiplicity, frequencies, linewidths = (
            self.get_mesh_frequencies_and_linewidths()
        )
        ...
        plt.axhline(y=0, color="k", ls="dashed")
        plt.title(
            "Frequency vs linewidths (from mesh: {})".format(
                self.parameters.mesh_phonopy
            )
        )
    ...
    def get_renormalized_phonon_dispersion_bands(
        self, with_linewidths=False, interconnect_bands=False, band_connection=False
    ):
        ...
        def eigenvector_order(ev_bands, ev_renormalized):
            metric = np.zeros_like(
                np.abs(np.dot(ev_bands[0].conjugate().T, ev_renormalized[0]))
            )
        ...
        _bands = pho_interface.obtain_phonon_dispersion_bands(
        ...
        _renormalized_bands = pho_interface.obtain_phonon_dispersion_bands(
            self.dynamic.structure,
            band_ranges,
        ...
        sup_lim = pho_interface.get_renormalized_force_constants(
            renormalized_frequencies + linewidths / 2,
            eigenvectors,
            self.dynamic.structure,
            fc_supercell,
        ...
        if with_linewidths:
            renormalized_bands_s = pho_interface.obtain_phonon_dispersion_bands(
                self.dynamic.structure,
                band_ranges,
            ...
            renormalized_bands_i = pho_interface.obtain_phonon_dispersion_bands(
                self.dynamic.structure,
                band_ranges,
        ...
        for i, q_path in enumerate(_bands[1]):
            ...
            band = {
                "q_path_distances": q_path.tolist(),
                "q_bounds": {
                    "inf": list(band_ranges[i][0]),
                    "sup": list(band_ranges[i][1]),
                },
                "harmonic_frequencies": {
                    "branch_{}".format(key): value.tolist()
                    for (key, value) in enumerate(_bands[2][i].T)
                },
                "renormalized_frequencies": {
                    "branch_{}".format(key): value.tolist()
                    for (key, value) in enumerate(_renormalized_bands[2][i].T)
                },
                "frequency_shifts": {
                    "branch_{}".format(key): value.tolist()
                    for (key, value) in enumerate(
                        _renormalized_bands[2][i].T - _bands[2][i].T
                    )
                },
            }
                ...
                band.update(
                    {
                        "linewidth_minus": {
                            "branch_{}".format(key): value.tolist()
                            for (key, value) in enumerate(renormalized_bands_i[2][i].T)
                        },
                        "linewidth_plus": {
                            "branch_{}".format(key): value.tolist()
                            for (key, value) in enumerate(renormalized_bands_s[2][i].T)
                        },
                        "linewidth": {
                            "branch_{}".format(key): value.tolist()
                            for (key, value) in enumerate(
                                renormalized_bands_s[2][i].T
                                - renormalized_bands_i[2][i].T
                            )
                        },
                    }
                )
    ...
    def get_mesh_frequencies_and_linewidths(self):
        ...
        linewidths_fc = pho_interface.get_renormalized_force_constants(
            linewidths,
            eigenvectors,
            self.dynamic.structure,
            fc_supercell,
        ...
        _, _, linewidths_mesh = pho_interface.obtain_phonopy_mesh_from_force_constants(
            self.dynamic.structure,
        ...
        frequencies_fc = pho_interface.get_renormalized_force_constants(
            renormalized_frequencies,
            eigenvectors,
            self.dynamic.structure,
            fc_supercell,
        ...
        qpoints, multiplicity, frequencies_mesh = (
            pho_interface.obtain_phonopy_mesh_from_force_constants(
                self.dynamic.structure,
    ...
    def print_phonon_dispersion_bands(self):
        ...
        for i, freq in enumerate(self._bands[1]):
            print(
                str(np.hstack([self._bands[1][i][None].T, self._bands[2][i]]))
                .replace("[", "")
                .replace("]", "")
            )

    def plot_eigenvectors(self):
        modes.plot_phonon_modes(
            self.dynamic.structure,
    ...
    def plot_dos_phonopy(self, force_constants=None):
        ...
        phonopy_dos = pho_interface.obtain_phonopy_dos(
            self.dynamic.structure,
            mesh=self.parameters.mesh_phonopy,
        ...
        if force_constants is not None:
            phonopy_dos_r = pho_interface.obtain_phonopy_dos(
                self.dynamic.structure,
                mesh=self.parameters.mesh_phonopy,
    ...
    def get_vc(self):
        if self._vc is None:
            ...
            if self._traj_file is not None:
                self._vc = projection.project_onto_wave_vector_lammps_vel(
                    self._traj_file,
                    self.dynamic,
                    [self.get_q_vector()],
                    project_on_atom=self.parameters.project_on_atom,
                ...
                self.dynamic.set_time(
                    np.linspace(
                        0,
                        (self._vc.shape[0] - 1) * self.dynamic.get_time_step_average(),
                        num=self._vc.shape[0],
                    )
                )
            else:
                self._vc = projection.project_onto_wave_vector(
                    self.dynamic,
                    self.get_q_vector(),
                    project_on_atom=self.parameters.project_on_atom,
                )
    ...
    def get_vq(self):
        if self._vq is None:
            ...
            self._vq = projection.project_onto_phonon(
                self.get_vc(), self.get_eigenvectors()
            )
        return self._vq
    ...
    def plot_vq(self, modes=None):
        ...
        time = np.linspace(
            0,
            self.get_vc().shape[0] * self.dynamic.get_time_step_average(),
    ...
    def save_vc(self, file_name, atom=1):
        ...
        if not 0 < atom <= self.get_vc().shape[1]:
            raise Exception(
                "Atom number {} does not exist in primitive cell".format(atom)
            )
    ...
    def get_power_spectrum_phonon(self):
        if self._power_spectrum_phonon is None:
            ...
            if self.parameters.use_symmetry:
                ...
                q_points_equivalent = pho_interface.get_equivalent_q_points_by_symmetry(
                    self.get_reduced_q_vector(), self.dynamic.structure
                )
                    ...
                    power_spectrum_phonon.append(
                        (
                            power_spectrum_functions[
                                self.parameters.power_spectra_algorithm
                            ]
                        )[0](self.get_vq(), self.dynamic, self.parameters)
                    )
    ...
    def get_power_spectrum_wave_vector(self):
        ...
        if self._power_spectrum_wave_vector is None:
            ...
            if self.parameters.use_symmetry:
                ...
                self._power_spectrum_wave_vector = np.average(
                    power_spectrum_wave_vector, axis=0
                )
    ...
    def get_power_spectrum_full(self, projection_on_coordinate=-1):
        ...
        if self._power_spectrum_direct is None:
            ...
            if projected_atom_type >= 0:
                print(
                    "Power spectrum projected onto atom type {0}".format(
                        projected_atom_type
                    )
                )
                supercell = self.dynamic.get_supercell_matrix()
                atom_types = np.array(
                    self.dynamic.structure.get_atom_type_index(supercell=supercell)
                )
                ...
                if projection_on_coordinate >= number_of_dimensions:
                    print(
                        "Projected coordinate should be smaller than {}".format(
                ...
                if projection_on_coordinate > -1:
                    print(
                        "Power spectrum projected onto coordinate {}".format(
                            projection_on_coordinate
                        )
                    )
                    velocity_mass_average = velocity_mass_average[
                        :, atom_indices, projection_on_coordinate, None
                    ]
                else:
            ...
            if self.parameters.silent:
                self._power_spectrum_direct = np.zeros_like(
                    self.parameters.frequency_range[None].T
                )
    ...
    def get_power_spectrum_partials(self, save_to_file=None):
        ...
        if self._power_spectrum_partials is None:
            ...
            self._power_spectrum_partials = (
                power_spectrum_functions[self.parameters.power_spectra_algorithm]
            )[0](velocity_mass_average[:, :, 0], self.dynamic, self.parameters)

            for i in [1, 2]:
                self._power_spectrum_partials += (
                    power_spectrum_functions[self.parameters.power_spectra_algorithm]
                )[0](velocity_mass_average[:, :, i], self.dynamic, self.parameters)

        if save_to_file is not None:
            np.savetxt(
                save_to_file,
                np.hstack(
                    [self.get_frequency_range()[None].T, self._power_spectrum_partials]
                ),
            )
    ...
    def phonon_width_scan_analysis(self):
        ...
        print(
            "Phonon coefficient scan analysis(Maximum entropy method/Symmetric Lorentzian fit only)"
        )
    ...
    def phonon_individual_analysis(self):
        ...
        # SW: changed to return the resulting values for easier automation
        return fitting.phonon_fitting_analysis(
    ...
    def plot_power_spectrum_full(self):
        ...
        ax1.plot(
            ...
            self.get_power_spectrum_full(),
            "r-",
            label="Power spectrum (MD)",
        )
        ...
        if self.dynamic.structure.forces_available():
            ...
            ax2.plot(
                phonopy_dos[0], phonopy_dos[1], "b-", label="DoS (Lattice dynamics)"
            )
        ...
        plt.legend(
            handles, ["Power spectrum (MD)", "DoS (Harmonic)", "DoS (Renormalized)"]
        )
        # plt.legend()
        plt.show()

        total_integral = integrate.simps(
            self.get_power_spectrum_full(), x=self.get_frequency_range()
        )
    ...
    def plot_power_spectrum_wave_vector(self):
        ...
        plt.plot(
            self.get_frequency_range(), self.get_power_spectrum_wave_vector(), "r-"
        )
        plt.xlabel("Frequency [THz]")
        plt.ylabel("eV * ps")
        plt.axhline(y=0, color="k", ls="dashed")
        plt.show()
        total_integral = integrate.simps(
            self.get_power_spectrum_wave_vector(), x=self.get_frequency_range()
        )
        print("Total Area (Kinetic energy <K>): {0} eV".format(total_integral))
    ...
    def plot_trajectory(self, atoms=None, coordinates=None):
        ...
        time = np.linspace(
            0,
            self.dynamic.trajectory.shape[0] * self.dynamic.get_time_step_average(),
    ...
    def plot_velocity(self, atoms=None, coordinates=None):
        ...
        time = np.linspace(
            0,
            self.dynamic.velocity.shape[0] * self.dynamic.get_time_step_average(),
    ...
    def plot_trajectory_distribution(self, direction):
        ...
        atom_type_index_unique = np.unique(
            self.dynamic.structure.get_atom_type_index(), return_index=True
        )[1]
        ...
        for atom in range(distributions.shape[0]):
            ...
            try:
                ...
                fitting_function = Gaussian_function(
                    distance_centers,
                    distributions[atom],
                    guess_height=1,
                    guess_position=0,
                ...
                print(
                    "Mean               {0:15.6f} Angstrom".format(
                        parameters["peak_position"]
                    )
                )
                print(
                    "Standard deviation {0:15.6f} Angstrom".format(parameters["width"])
                )
                print("Global fit error   {0:15.6f}".format(parameters["global_error"]))

                plt.plot(
                    distance,
                    fitting_function.get_curve(distance),
            ...
            plt.xlabel(
                "Direction: " + " ".join(np.array(direction, dtype=str)) + " [Angstrom]"
            )
    ...
    def write_power_spectrum_phonon(self, file_name):
        reading.write_curve_to_file(
            self.get_frequency_range(), self.get_power_spectrum_phonon(), file_name
        )
    ...
    def get_atomic_displacements(self, direction):
        ...
        for atom in range(projections.shape[0]):
            distribution, bins = np.histogram(
                projections[atom],
                bins=number_of_bins,
    ...
    def get_temperature(self):
        if not self._temperature:
            ...
            self._temperature = energy.boltzmann_distribution(
    ...
    def get_commensurate_points_data(self, auto_range=True, qlist=None):
        # Sandro Wieser: added option to compute specific q points instead of just all commensurate ones
        # obviously, the q points given should still be commensurate with the supercell!
        ...
        if self._commensurate_points_data is None:
            ...
            if auto_range:
                ...
                phonopy_dos = pho_interface.obtain_phonopy_dos(
                    self.dynamic.structure, mesh=self.parameters.mesh_phonopy
                )

                self.set_frequency_limits([0, np.max(phonopy_dos[0][-1]) * 1.2])
                print(
                    "set frequency range: {} - {}".format(
                        self.get_frequency_range()[0], self.get_frequency_range()[-1]
                    )
                )
            ...
            if qlist is None:
                com_points = pho_interface.get_commensurate_points(
                    self.dynamic.structure, fc_supercell
                )
            else:
                com_points = qlist
            ...
            for i, reduced_q_vector in enumerate(com_points):
                ...
                q_points_equivalent = pho_interface.get_equivalent_q_points_by_symmetry(
                    reduced_q_vector, self.dynamic.structure
                )
            ...
            if self.parameters.save_renormalized_frequencies:
                print(
                    "This option will be deprecated in the future. Please use save quasiparticle data option"
                )
                np.savetxt("renormalized_frequencies", renormalized_frequencies)
            # np.savetxt('test_line', linewidths)

            self._commensurate_points_data = {
                "frequencies": renormalized_frequencies,
    ...
    def get_commensurate_points_properties(self, qlist=None):
        ...
        for i, reduced_q_vector in enumerate(com_points):
            ...
            gv = pho_interface.obtain_phonopy_group_velocity(
                self.dynamic.structure,
                reduced_q_vector,
        ...
        return {
            "group_velocity": group_velocity_list,
            "q_points": q_points_list,
            "fc_supercell": fc_supercell,
        }
    ...
    def write_quasiparticles_data(
        self, filename="quasiparticles_data.yaml", with_extra=False
    ):
    ...
    def write_quasiparticles_data_at_q(
        self, filename="quasiparticles_data_q.yaml", with_extra=False, qpoint=[0, 0, 0]
    ):
        quasiparticle_data = self.get_commensurate_points_data(qlist=[qpoint])
        if with_extra:
            quasiparticle_data.update(
                self.get_commensurate_points_properties(qlist=[qpoint])
            )
        reading.save_quasiparticle_data_to_file(quasiparticle_data, filename)
    ...
    def get_thermal_properties(self, force_constants=None):
        ...
        if force_constants is not None:
            ...
            phonopy_dos_h = pho_interface.obtain_phonopy_dos(
                self.dynamic.structure,
                mesh=self.parameters.mesh_phonopy,
                freq_min=0.01,
                freq_max=self.get_frequency_range()[-1],
            ...
            free_energy += thm.get_free_energy_correction_dos(
                temperature, phonopy_dos_h[0], phonopy_dos_h[1], phonopy_dos[1]
            )
            total_energy += thm.get_free_energy_correction_dos(
                temperature, phonopy_dos_h[0], phonopy_dos_h[1], phonopy_dos[1]
            )
    ...
    def display_thermal_properties(
        self, from_power_spectrum=False, normalize_dos=False, print_phonopy=False
    ):
        ...
        if print_phonopy:
            harmonic_properties = pho_interface.obtain_phonopy_thermal_properties(
                self.dynamic.structure,
                temperature,
                mesh=self.parameters.mesh_phonopy,
                NAC=self.parameters.use_NAC,
            ...
            renormalized_properties = pho_interface.obtain_phonopy_thermal_properties(
                self.dynamic.structure,
                temperature,
                mesh=self.parameters.mesh_phonopy,
                force_constants=self.get_renormalized_force_constants(),
            ...
            print(
                "Free energy (not corrected):   {0:.4f}       {3:.4f}     KJ/mol\n"
        ...
        renormalized_properties = self.get_thermal_properties(
            force_constants=self.get_renormalized_force_constants()
        )
        ...
        if from_power_spectrum:
            ...
            power_spectrum_dos = thm.get_dos(
                temperature,
                frequency_range,
                self.get_power_spectrum_full(),
                normalization,
            ...
            if normalize_dos:
                ...
                if self.parameters.project_on_atom > -1:
                    power_spectrum_dos /= (
                        self.dynamic.structure.get_number_of_primitive_atoms()
                    )
                    integration /= (
                        self.dynamic.structure.get_number_of_primitive_atoms()
                    )

            free_energy = thm.get_free_energy(
                temperature, frequency_range, power_spectrum_dos
            )
            ...
            total_energy = thm.get_total_energy(
                temperature, frequency_range, power_spectrum_dos
            )
            ...
            print(
                "                             Harmonic   Quasiparticle   Power spectrum\n"
            )
            print(
                "Free energy   (KJ/mol): {0:12.4f}  {5:12.4f}  {10:12.4f}\n"
            ...
            print("                            Harmonic    Quasiparticle\n")
            print(
                "Free energy   (KJ/mol): {0:12.4f}  {5:12.4f}\n"
        ...
        if not self.parameters.silent:
            self.plot_dos_phonopy(
                force_constants=self.get_renormalized_force_constants()
            )

    def get_anisotropic_displacement_parameters(
        self, coordinate_type="uvrs", print_on_screen=True
    ):
        ...
        average_positions = self.dynamic.get_mean_displacement_matrix(
            use_average_positions=True
        )
        if print_on_screen:
            print(
                "Anisotropic displacement parameters ({0}) [relative to average atomic positions]".format(
                    coordinate_type
                )
            )
            print(
        ...
        for i, u_cart in enumerate(average_positions):
            ...
            n = np.array(
                [
                    [np.linalg.norm(cell_inv.T[0]), 0, 0],
            ...
            u = {"cart": u_cart, "crys": u_crys, "uvrs": u_uvrs}
    ...
    def get_average_atomic_positions(self, to_unit_cell=True):
        ...
        for i, coordinate in enumerate(positions_average):
            print(
                "{0:2} ".format(elements[i])
                + "{0:15.8f} {1:15.8f} {2:15.8f}".format(*coordinate.real)
            )

    # QHA methods
    def set_qha_force_constants(self, fc_qha_file):
        self._force_constants_qha = pho_interface.get_force_constants_from_file(
            fc_qha_file, fc_supercell=self.dynamic.structure.get_supercell_phonon()
        )
    ...
    def get_qha_shift(self, reduced_q_vector):
        if self._force_constants_qha is not None:
            ...
            qha_frequencies = pho_interface.obtain_eigenvectors_and_frequencies(
                structure_qha, reduced_q_vector, print_data=False
            )[1]
